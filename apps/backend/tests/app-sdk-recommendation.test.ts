...
describe("Phase 8.1: SDK Recommendation & Integration Choice", () => {
  ...
  describe("Backend Service Layer", () => {
    ...
    it("should reject manually enabling the SDK without a verified handshake", async () => {
      ...
      await expect(
        ...
        code: "BAD_REQUEST",
        message: "SDK_ENABLED can only be confirmed by a valid SDK handshake",
      });
    });
  });

  describe("API Endpoints & RBAC / IDOR Protection", () => {
    it("PATCH /apps/:id/sdk-status: Owner developer can choose manual verification", async () => {
      ...
      expect(body.sdkIntegrationStatus).toBe("NO_SDK");
      expect(body.hasCallTestSdk).toBe(false);
    });
